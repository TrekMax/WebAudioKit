...
export function EqCurveEditor({ gainsDb, onChange }: EqCurveEditorProps) {
  const svgRef = useRef<SVGSVGElement>(null)
  const dragSessionRef = useRef<EqDragSession | null>(null)
  const [selectedBandIndex, setSelectedBandIndex] = useState(3)
  const points = useMemo(() => buildEqCurvePoints(gainsDb), [gainsDb])
  const curvePath = useMemo(() => buildEqCurvePath(points), [points])
  const selectedFrequency = EQ_BAND_FREQUENCIES_HZ[selectedBandIndex] ?? 1_000
  ...
  const updateBand = (bandIndex: number, gainDb: number) => {
    const next = EQ_BAND_FREQUENCIES_HZ.map((_, index) => (
      index === bandIndex
  ...
  return (
    <div className="eq-curve-editor">
      ...
      >
        ...
        {points.map((point, index) => (
          <circle
            ...
            onFocus={() => setSelectedBandIndex(index)}
            onKeyDown={(event) => handlePointKeyDown(event, index)}
          />
        ))}
      </svg>

      <div className="eq-band-selector" role="group" aria-label="选择 EQ 频段">
        {EQ_BAND_FREQUENCIES_HZ.map((frequencyHz, index) => (
          <button
            key={frequencyHz}
            type="button"
      ...
      <button
        ...
        onClick={() => onChange(EQ_BAND_FREQUENCIES_HZ.map(() => 0))}
      >复位为平直曲线</button>
    </div>
  )
}
