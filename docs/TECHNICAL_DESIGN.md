...
- **决定**：采样器节点在 AudioWorklet 内执行固定成本的抗混叠与抽取，并可选择零阶保持或分段线性重建，始终输出 AudioContext 的固定采样率。
