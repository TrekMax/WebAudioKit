...
左侧节点库为全部节点类型提供由指针悬停或键盘焦点触发的只读说明面板。面板复用纯数据的确定性示例，不读取 PCM、不创建 AudioContext 或触发 FFT/STFT：Biquad 幅度塑形与七段 EQ 使用对数频率/dBFS 二维频谱叠加图，全通使用归一化波形说明相位偏移，采样器使用处理前后波形说明低采样率重建与高频细节抑制。面板必须标记为示意而非当前音频分析，并通过与其他邻近面板相同的边界计算钳位在节点工作区；图表同时提供文字摘要，键盘聚焦时通过 `aria-describedby` 关联。
