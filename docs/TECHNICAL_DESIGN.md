...
每声道增益的可听规则为 `!muted[c] && (!anySolo || solo[c])`，因此 Mute 优先于 Solo。输出节点的悬浮控制在该规则之外再乘监听系数：左声道 `c=0` 在 Balance 偏右时按 `1-balance` 衰减，右声道 `c=1` 在 Balance 偏左时按 `1+balance` 衰减，各自的启用状态可将系数置零；`c>=2` 不受 Balance 影响。所有增益变化使用短线性斜坡，不重建播放 session。路由保持源索引的身份顺序；5.1 标签顺序为 `FL, FR, FC, LFE, BL, BR`，7.1 作为显式扩展追加 `SL, SR`。语义布局是项目元数据和 UI 标识，不承诺浏览器、操作系统或物理设备提供相同数量的扬声器，也不执行任意硬件端口映射。

播放图不依赖 `AnalyserNode` 生成权威 FFT，以保证实时与离线频谱使用相同窗函数和幅度标定。Mute/Solo 不回写源 PCM，也不进入分析或导出管线。

滤波选项页可在 Merger 后编译一条有序监听效果链。基础滤波编译为 BiquadFilterNode；EQ 曲线节点可选择固定的 7、10 或 15 段预设，并按中心频率顺序展开为相同数量的 Peaking BiquadFilterNode。7 段预设为 60 Hz、150 Hz、400 Hz、1 kHz、2.5 kHz、6 kHz 与 16 kHz，Q 为 1.1；默认 10 段倍频程预设为 31.5 Hz、63 Hz、125 Hz、250 Hz、500 Hz、1 kHz、2 kHz、4 kHz、8 kHz 与 16 kHz，Q 为 √2；15 段 2/3 倍频程预设为 25 Hz、40 Hz、63 Hz、100 Hz、160 Hz、250 Hz、400 Hz、630 Hz、1 kHz、1.6 kHz、2.5 kHz、4 kHz、6.3 kHz、10 kHz 与 16 kHz，Q 为 2.15。声明式配置保存预设段数与各段 ±24 dB 增益，切换段数时按对数频率线性插值已有增益。运行路由同时维护声明式节点到一个或多个 AudioNode 的分组映射，因此同一预设内拖动曲线只原位更新对应参数；段数变化时重建当前效果路由但不重建播放源，类型、启用状态或链路结构变化也沿用相同的有界替换流程；频率响应预览累加组内全部 Biquad 响应。采样器编译为 AudioWorkletNode，在输出上下文固定采样率内模拟较低目标采样率：“原始抽点”由相位累加器直接选取当前输入点并零阶保持，故意不做抗混叠；其余模式的输入先经过有界一阶低通抗混叠，再由相位累加器抽取，并以零阶保持、因果线性插值、四点 Catmull–Rom 或 128 相位 16 抽头窗化 sinc 重建到上下文帧率。三次模式使用两个目标采样间隔的因果历史；带限模式使用八个目标采样间隔的固定延迟。算法作为 Worklet 构造配置，切换时替换当前监听路由，目标采样率仍以 k-rate AudioParam 原位更新；频响预览将“原始抽点”表示为不含抗混叠低通的对照基线，其他模式分别使用保持、`sinc²`、三次插值平均响应或带限过渡带近似。该幅频近似无法表达折叠到其他频率的混叠分量，界面文案必须明确它只是预测。目标采样率不低于上下文采样率时界面显示“透明直通”、禁用算法选择并直接复制输入，且不虚构新的 PCM 帧或超过上下文 Nyquist 的信息。Worklet 构造时预分配最多 32 声道、每声道 16 个历史样本的状态，并只为带限模式构建固定 128×16 系数表；`process()` 不执行 I/O、日志、Promise、动态导入、对象或 TypedArray 分配、数组扩容或无界循环；能力缺失或模块加载失败时以透明 GainNode 旁路。

干声与湿声分支同时连接到 Master Gain，A/B 试听仅对两条分支的 GainNode 做短斜坡切换，不重建 AudioBufferSourceNode。节点增删、排序、类型或参数变化时在主线程控制路径创建新链，切换连接后断开并释放旧节点；只有采样器的固定成本逐采样内核进入实时渲染回调。效果链只属于监听图，不进入原始 PCM、权威 FFT/STFT、峰值和导出管线。双声轨波形模式对 A 使用当前时间视口内的源 PCM 有界取样，对 B 使用活动下采样采样器中最低目标采样率节点构建主导时域近似：源 PCM 先以线性插值近似 AudioBufferSourceNode 到输出上下文的原生采样率适配，再按直接抽点或抗混叠后的保持、线性、三次、带限重建规则匹配 Worklet；一阶低通只用于非原始抽点模式，且只从每个可视取样点前执行最多 64 个上下文采样帧的因果预热，不从文件开头扫描。频谱模式对 A 使用当前位置源 STFT，对 B 叠加 BiquadFilterNode 的实际幅频响应与采样器所选算法的抗混叠及重建近似。二维声谱模式复用有界降采样后的离线源 STFT，并将相同响应逐频率 bin 叠加到 B 轨；B 轨三种视图均为监听预测，不将其冒充为效果后权威 PCM/STFT。坐标轴由纯数据轴模型生成，并与绘制共用范围和映射：波形为当前时间视口/归一化幅度，频谱为 20 Hz 至 Nyquist 的对数频率/dBFS，二维声谱为当前 STFT 时间视口/线性频率；A/B 两轨使用完全相同的刻度以支持直接比较。时间视图以整数采样点保存共享范围，滚轮缩放围绕指针锚点并钳位到源或分析时域；点击只在绘图区映射到采样点，并通过既有 PlaybackController seek，不在频谱的频率横轴上提供时间跳转。波形预览列数不超过当前绘图区 CSS 像素数与 1200 的较小值，每列最多读取 8 个源取样；放大到每列不超过 8 个采样点时额外连接中点折线，以显示五种采样与重建差异。声谱缩放裁剪既有有界位图，不复制完整 PCM 或触发新 STFT。A/B 波形概览、滤波响应、声谱像素与离屏位图使用源 `AudioBuffer` 或 STFT 结果对象作为弱引用缓存所有者，并以时间范围、列数、采样器配置和滤波版本作为有界 LRU 键；因此切换预览模式或工作台页面可复用派生结果，关闭资源后又不会由缓存阻止 PCM/STFT 回收。

左侧节点库为全部节点类型提供由指针悬停或键盘聚焦触发的只读说明面板。面板复用纯数据的确定性示例，不读取 PCM、不创建 AudioContext 或触发 FFT/STFT：Biquad 幅度塑形与多档 EQ 使用对数频率/dBFS 二维频谱叠加图，全通使用归一化波形说明相位偏移，采样器使用处理前后波形说明低采样率重建与高频细节抑制。面板必须标记为示意而非当前音频分析，并通过与其他邻近面板相同的边界计算钳位在节点工作区；图表同时提供文字摘要，键盘聚焦时通过 `aria-describedby` 关联。独立“音频知识图谱”页面按知识分类渲染相同的节点说明内容组件与图表模型，不复制第二份节点文案或示例算法；节点分类目录由只包含 `FilterKind` 的静态目录声明，并以单元测试确保每个已支持节点恰好出现一次。核心原理使用独立的纯数据目录描述 IIR、奈奎斯特采样定理、三次 Catmull–Rom 重建、窗化 sinc 带限重建、振幅包络、Q/带宽、dBFS 和 FFT/STFT，并配套不依赖运行时音频的内联 SVG 教学图例及关联主题标签；三次图例对比四点平滑曲线与线性连接，带限图例展示 16 抽头核与 128 分数相位。单元测试验证必需概念、唯一标识和文案完整性。知识卡片宽屏以 450 px 为最小列宽自动排布，在 1180 px 及以下强制单列，使 SVG 坐标、正文与参数摘要获得稳定的可读尺寸。知识图谱属于纯展示层，切入时实时频谱需求判定返回 false，避免不可见视图继续调度 FFT。

常用滤波预设由音频领域层的静态声明式目录定义，应用时展开为既有 `FilterNodeConfig[]` 并追加到当前链路；预设不引入新的运行时节点类型，也不覆盖已有配置。展开结果继续经过相同的频率钳位、节点数量限制、链路校验和 Web Audio 编译路径，用户可对生成节点逐个编辑、排序、旁路或删除。

节点参数编辑器以画布内悬浮面板呈现，不占用固定第三栏。面板位置由所选节点、画布滚动位置和可用边界计算，优先放在节点右侧、空间不足时翻转到左侧，并钳位在节点工作区；关闭面板只清除临时 UI 选中状态，不改变声明式节点配置。中间节点排序使用 Pointer Events，并将 pointer capture 保持在不会随排序移动的画布容器上；拖动期间只在组件内维护 ID 顺序预览并支持画布边缘自动滚动。原节点作为布局占位，独立悬浮副本按画布滚动坐标实时跟随指针并钳位在可视区域，通过有界缩放和倾斜提供弹性反馈。`pointerup` 才把最终数组一次性传给 `setFilterChain()`，`pointercancel` 或 `Esc` 丢弃预览，避免移动过程中反复重建 Web Audio 图。输入终端可点击复用同一悬浮面板结构，以只读字段展示当前活动资源的文件元数据与解码 PCM 摘要，不读取或复制 PCM 内容。输出终端另提供悬浮按钮与邻近控制面板，复用相同的翻转、钳位和滚动重定位规则，并直接发送总音量、左右启用和 Balance 命令到播放引擎；L/C/R 快捷按钮分别发送 `-1/0/1`，其中 C 用于居中复位。单声道界面禁用右声道与 Balance，多声道只将其解释为前两个输出声道的监听系数。
...
- **决定**：采样器节点在 AudioWorklet 内执行固定成本的抽取；“原始抽点”直接取点且不做抗混叠，其余模式先执行一阶抗混叠，再选择零阶保持、因果线性、四点三次或固定 16 抽头带限重建。节点始终输出 AudioContext 的固定采样率；目标不低于上下文时透明直通。
- **原因**：Web Audio 中间节点不能改变上下文输出采样率；监听效果仍需保持播放时长、源 PCM 和导出语义不变。
- **代价**：节点不能执行真正改变 PCM 帧率的上采样；三次与带限模式增加固定因果延迟，带限模式的实时乘加成本最高，且该节点不是导出重采样功能。

### ADR-013：EQ 曲线节点使用固定预设 Biquad 分组

- **状态**：Accepted
- **记录**：[`docs/adr/013-graphic-equalizer-node.md`](adr/013-graphic-equalizer-node.md)
- **决定**：一个声明式 EQ 节点按所选 7、10 或 15 段预设展开为串联 Peaking BiquadFilterNode，默认 10 段；同档增益通过运行时分组映射原位更新，切换档位时按对数频率插值并替换分组。
