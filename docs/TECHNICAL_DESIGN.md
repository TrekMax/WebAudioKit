...
左侧节点库为全部节点类型提供由指针悬停或键盘焦点触发的只读说明面板。面板复用纯数据的确定性示例，不读取 PCM、不创建 AudioContext 或触发 FFT/STFT：Biquad 幅度塑形与多档 EQ 使用对数频率/dBFS 二维频谱叠加图，全通使用归一化波形说明相位偏移，采样器使用处理前后波形说明低采样率重建与高频细节抑制。面板必须标记为示意而非当前音频分析，并通过与其他邻近面板相同的边界计算钳位在节点工作区；图表同时提供文字摘要，键盘聚焦时通过 `aria-describedby` 关联。独立“音频知识图谱”页面按知识分类渲染相同的节点说明内容组件与图表模型，不复制第二份节点文案或示例算法；节点分类目录由只包含 `FilterKind` 的静态目录声明，并以单元测试确保每个已支持节点恰好出现一次。核心原理使用独立的纯数据目录描述 IIR、奈奎斯特采样定理、振幅包络、Q/带宽、dBFS 和 FFT/STFT，并配套不依赖运行时音频的内联 SVG 教学图例及关联主题标签；单元测试验证必需概念、唯一标识和文案完整性。知识卡片宽屏以 450 px 为最小列宽自动排布，在 1180 px 及以下强制单列，使 SVG 坐标、正文与参数摘要获得稳定的可读尺寸。知识图谱属于纯展示层，切入时实时频谱需求判定返回 false，避免不可见视图继续调度 FFT。
