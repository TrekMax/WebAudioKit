...
节点参数编辑器以画布内悬浮面板呈现，不占用固定第三栏。面板位置由所选节点、画布滚动位置和可用边界计算，优先放在节点右侧、空间不足时翻转到左侧，并钳位在节点工作区；关闭面板只清除临时 UI 选中状态，不改变声明式节点配置。输出终端另提供悬浮按钮与邻近控制面板，复用相同的翻转、钳位和滚动重定位规则，并直接发送总音量、左右启用和 Balance 命令到播放引擎；L/C/R 快捷按钮分别发送 `-1/0/1`，其中 C 用于居中复位。单声道界面禁用右声道与 Balance，多声道只将其解释为前两个输出声道的监听系数。
