...
- **决定**：采样器节点在 AudioWorklet 内执行固定成本的抗混叠与抽取，并可选择零阶保持、因果线性、四点三次或固定 16 抽头带限重建，始终输出 AudioContext 的固定采样率；目标不低于上下文时透明直通。
